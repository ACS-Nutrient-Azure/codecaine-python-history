...
class RecordService:
    ...
    async def upsert_record(self, db: AsyncSession, req: RecordUpsertRequest) -> None:
        ...
        # 이 supplement의 기존 purchase_history 조회 (supplement당 1개 유지)
        ...
        if diff > 0:
            ...
            if ph_row:
                # 이미 purchase_history 있으면 remain_day만 차감
                ph_row.remain_day = max(0, (ph_row.remain_day or 0) - diff)
            else:
                # 최초 섭취: supplement 정보로 remain_day 초기 계산
                supp_result = await db.execute(
            ...
            # 섭취 취소: purchase_history의 item_id는 보호 (FK 보전)
